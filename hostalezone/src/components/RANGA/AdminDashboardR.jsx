...
// ═══════════════════════════════════════════════════════════════════════════════
function LeaveManagement({ token, leaves, setLeaves, refreshLeaves }) {
  ...
  const updateStatus = async (id, newStatus) => {
    ...
    try {
      ...
      setSelected(null); setComment(""); loadLeaves(); if (refreshLeaves) refreshLeaves();
    } catch { showLToast("Network error", "error"); }
    finally { setActLoading(false); }
  };

  const deleteLeaveRequest = async (id) => {
    if (!window.confirm("Are you sure you want to delete this leave request permanently?")) return;
    setLoading(true);
    try {
      ...
      if (res.ok) {
        showLToast("Leave request deleted successfully.");
        loadLeaves();
        if (refreshLeaves) refreshLeaves();
      } else {
        const d = await res.json();
        showLToast(d.message || "Failed to delete", "error");
      }
    } catch {
      showLToast("Network error", "error");
    } finally {
      setLoading(false);
    }
  };
                  ...
                  <tr key={l._id} style={{ borderBottom: "1px solid rgba(255,255,255,.03)", transition: "background .12s" }}>
                    <td style={{ padding: "13px 16px" }}>
                      <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                        <Avatar name={l.student?.fullName} src={getPhotoUrl(l.student)} />
                        <div>
                    ...
                    <td style={{ padding: "13px 16px" }}>
                      <div style={{ display: "flex", gap: 8 }}>
                        ...
                        <button onClick={() => deleteLeaveRequest(l._id)} style={{ padding: "7px 11px", borderRadius: 8, border: "1px solid rgba(239,68,68,.2)", background: "rgba(239,68,68,.06)", color: "#fca5a5", cursor: "pointer", transition: "all .2s", display: "inline-flex", alignItems: "center", justifyContent: "center" }} title="Delete Request">
                          <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><polyline points="3 6 5 6 21 6" /><path d="M19 6l-1 14a2 2 0 0 1-2 2H8a2 2 0 0 1-2-2L5 6" /><path d="M10 11v6M14 11v6" /><path d="M9 6V4a2 2 0 0 1 2-2h2a2 2 0 0 1 2 2v2" /></svg>
                        </button>
                      </div>
                    </td>
                  </tr>
            ...
            <div style={{ padding: "22px 28px" }}>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 18 }}>
                <div style={{ background: "rgba(255,255,255,.03)", border: "1px solid rgba(255,255,255,.06)", borderRadius: 13, padding: "14px 16px" }}>
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                    <Avatar name={selected.student?.fullName} src={getPhotoUrl(selected.student)} size={40} />
                    <div>
...
export default function AdminDashboard() {
  ...
  const [leaves, setLeaves] = useState([]); // Lifted state
  const [search, setSearch] = useState("");
  ...
  useEffect(() => { if (!adminToken) navigate("/SignIn"); }, [adminToken, navigate]);
  useEffect(() => { if (activeMenu !== "students") { setViewStudent(null); setEditStudent(null); setDeleteStudent(null); } }, [activeMenu]);

  const handleLogout = () => { localStorage.removeItem("adminToken"); localStorage.removeItem("adminData"); navigate("/SignIn"); };
  ...
  const loadLeaves = async (statusFilter = "all") => {
    try {
      const res = await fetch(`${API_BASE}/api/admin/leaves?status=${encodeURIComponent(statusFilter)}`, { headers: { Authorization: `Bearer ${adminToken}` } });
      const data = await res.json();
      setLeaves(data.leaves || []);
    } catch { setLeaves([]); }
  };

  useEffect(() => { if (activeMenu === "students" || activeMenu === "dashboard") fetchStudents(); if (activeMenu === "leaves" || activeMenu === "dashboard") loadLeaves(); }, [activeMenu]);
  ...
  const saveEdit = async () => {
    try {
      ...
      const res = await axios.put(`${API_BASE}/api/admin/students/${editStudent._id}`, { itNumber: editStudent.itNumber, fullName: editStudent.fullName, email: editStudent.email, phone: editStudent.phone, department: editStudent.department, yearSemester: editStudent.yearSemester, address: editStudent.address }, axiosConfig);
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(400px, 1fr))', gap: '1.5rem', marginBottom: '1.5rem' }}>
        ...
        <div style={{ background: "rgba(255,255,255,.025)", border: "1px solid rgba(255,255,255,.06)", borderRadius: 18, overflow: 'hidden' }}>
          <div style={{ padding: '1.25rem 1.5rem', borderBottom: '1px solid rgba(255,255,255,0.05)', fontWeight: 700, color: '#f8fafc', fontSize: '0.875rem', fontFamily: "'Syne',sans-serif", display: 'flex', alignItems: 'center', gap: '0.6rem' }}>
          ...
          <div style={{ height: '300px', padding: '1.5rem' }}>
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={deptData} layout="vertical">
                ...
                <YAxis dataKey="name" type="category" width={100} tick={{ fontSize: 10, fill: 'rgba(248,250,252,.4)', fontWeight: 600 }} axisLine={false} tickLine={false} />
                <Tooltip cursor={{ fill: 'rgba(255, 255, 255, 0.05)' }} contentStyle={{ background: '#0d0d18', borderRadius: '12px', border: '1px solid rgba(255,255,255,0.1)', boxShadow: '0 10px 30px rgba(0,0,0,0.1)', fontSize: '12px' }} itemStyle={{ color: '#f8fafc' }} />
          ...
          <div style={{ height: '300px', padding: '1.5rem' }}>
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={registrationTrends}>
                ...
                <XAxis dataKey="month" axisLine={false} tickLine={false} tick={{ fontSize: 10, fill: 'rgba(248,250,252,.4)' }} />
                <Tooltip contentStyle={{ background: '#0d0d18', borderRadius: '12px', border: '1px solid rgba(255,255,255,0.1)', boxShadow: '0 10px 30px rgba(0,0,0,0.1)', fontSize: '12px' }} itemStyle={{ color: '#f8fafc' }} />
                <Area type="monotone" dataKey="count" stroke="#8b5cf6" strokeWidth={2.5} fillOpacity={1} fill="url(#adminGrad)" />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </div>
    ...
    <div>
      ...
      <div style={{ background: "rgba(255,255,255,.02)", border: "1px solid rgba(255,255,255,.06)", borderRadius: 18, overflow: "hidden" }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 14, padding: "18px 22px", borderBottom: "1px solid rgba(255,255,255,.05)" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
            <div style={{ position: "relative" }} className="ad-search-wrap">
            ...
            <table style={{ width: "100%", borderCollapse: "collapse" }}>
              ...
              <tbody>
                {students.map(s => {
                  ...
                  return (
                    <tr key={s._id} className="ad-table-row" style={{ borderBottom: "1px solid rgba(255,255,255,.03)" }}>
                      <td style={{ padding: "13px 16px" }}>
                        <div style={{ display: "flex", alignItems: "center", gap: 11 }}>
                          <Avatar name={s.fullName} src={getPhotoUrl(s)} />
                          <div>
                            <div style={{ fontWeight: 500, fontSize: 14, color: "#f1f5f9", marginBottom: 2 }}>{s.fullName || "—"}</div>
                            <div style={{ fontSize: 11, color: "rgba(248,250,252,.25)", fontWeight: 300 }}>{s.itNumber || "—"} · {formatDate(s.createdAt)}</div>
                          </div>
                        </div>
                      </td>
                      ...
                      <td style={{ padding: "13px 16px" }}>
                        <div style={{ display: "flex", gap: 5, flexWrap: "wrap" }}>
                          {[
                            ...
                            { label: "Edit", ac: "245,158,11", col: "#fcd34d", fn: () => setEditStudent({ _id: s._id, itNumber: s.itNumber || "", fullName: s.fullName || "", email: s.email || "", phone: s.phone || "", department: s.department || "", yearSemester: s.yearSemester || "", address: s.address || "", isActive: s.isActive !== false }) },
    ...
    <>
      <style>{`
        ...
        *,*::before,*::after{box-sizing:border-box;margin:0;padding:0;}
        ...
        .ad-table-row{transition:all .15s ease-in-out;}
        .ad-table-row:hover{background:rgba(255,255,255,.025);box-shadow:inset 4px 0 0 #6366f1;}
        
        .ad-search-wrap:focus-within{border-color:rgba(99,102,241,.4) !important;box-shadow:0 0 0 3px rgba(99,102,241,.06);}
      ...
      <div className="ad-shell">
        ...
        <div className={`ad-main${sideCollapsed ? " col" : ""}`}>
          ...
          <div className="ad-page">
            ...
            {activeMenu === "leaves" && <LeaveManagement token={adminToken} leaves={leaves} setLeaves={setLeaves} refreshLeaves={loadLeaves} />}
          </div>
        </div>
      </div>
          ...
          <div className="ad-modal" onClick={e => e.stopPropagation()}>
            ...
            <div className="ad-modal-body">
              <div className="ad-info-grid">
                {[
                  { label: "IT Number", value: viewStudent.itNumber || "—" },
          ...
          <div className="ad-modal" onClick={e => e.stopPropagation()}>
            ...
            <div className="ad-modal-body">
              <div className="ad-form-grid">
                {[
                  { label: "IT Number", key: "itNumber", full: false, ph: "IT23139312", type: "text" },
