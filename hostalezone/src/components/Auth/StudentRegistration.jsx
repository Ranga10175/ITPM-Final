...
function StudentRegister() {
  ...
  React.useEffect(() => {
    const newErrors = {};
    if (student.studentId && !/^IT\d{8}$/.test(student.studentId)) {
      newErrors.studentId = "Must start with 'IT' followed by exactly 8 digits.";
    }
    if (student.fullName && (student.fullName.length < 5 || student.fullName.length > 50)) {
      newErrors.fullName = "Student name must be between 5 and 20 characters."; // Follow the user's screenshot requirement
    } else if (student.fullName && !/^[a-zA-Z\s]+$/.test(student.fullName)) {
      newErrors.fullName = "Full Name must contain only English letters and spaces.";
    }
    ...
    if (student.password && (student.password.length < 3 || student.password.length > 6)) {
      newErrors.password = "Password must be between 3 and 6 characters.";
    }
  ...
  const handleChange = (e) => {
    ...
    if (name === "profilePhoto") {
      ...
      if (/^$|^I$|^IT$|^IT\d{0,8}$/.test(formattedValue)) {
        setStudent({ ...student, [name]: formattedValue });
      }
    } else if (name === "fullName") {
      // Restrict to English letters and spaces only
      ...
      // Restrict exactly to English letters, numbers, @, and .
      if (/^[a-zA-Z0-9@.]*$/.test(value)) {
        setStudent({ ...student, [name]: value });
      }
    } else if (name === "password" || name === "confirmPassword") {
      // Allow any characters up to 6 length
      if (value.length <= 6) {
        setStudent({ ...student, [name]: value });
      }
    } else {
  ...
  const validateForm = () => {
    ...
    const requiredFields = ['studentId', 'fullName', 'email', 'phone', 'password', 'confirmPassword', 'department', 'year', 'semester', 'address'];
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Save token and user data to localStorage to "log in" the user
      ...
      // Navigate to Student Dashboard after a brief delay to show the success popup
      setTimeout(() => {
  ...
  return (
    <>
      ...
      <div className="student-register">
        <div className="student-register-card">
          ...
          <form onSubmit={handleSubmit} encType="multipart/form-data" className="student-form">
            <div className="form-row">
              <div className="form-group">
                ...
                <input type="text" name="studentId" placeholder="ex: IT23139312"
                  value={student.studentId} onChange={handleChange} onBlur={handleBlur} required
                  className={touched.studentId && errors.studentId ? "error-input" : ""} />
                {touched.studentId && errors.studentId && <span className="error-text">{errors.studentId}</span>}
              </div>
              <div className="form-group">
                <label>Full Name</label>
                <input type="text" name="fullName" placeholder="Enter your full name"
                  value={student.fullName} onChange={handleChange} onBlur={handleBlur} required
            ...
            <div className="form-row">
              <div className="form-group">
                ...
                <input type="email" name="email" placeholder="your@email.com"
                  value={student.email} onChange={handleChange} onBlur={handleBlur} required
              ...
              <div className="form-group">
                ...
                <input type="text" name="phone" placeholder="Phone number"
                  value={student.phone} onChange={handleChange} onBlur={handleBlur} required
            ...
            <div className="form-row">
              <div className="form-group">
                ...
                <input type="password" name="password" placeholder="Create a password"
                  value={student.password} onChange={handleChange} onBlur={handleBlur} required
              ...
              <div className="form-group">
                ...
                <input type="password" name="confirmPassword" placeholder="Confirm password"
                  value={student.confirmPassword} onChange={handleChange} onBlur={handleBlur} required
            ...
            <div className="form-group">
              ...
              <select name="department" value={student.department} onChange={handleChange} onBlur={handleBlur} required
                className={touched.department && errors.department ? "error-input" : ""}
                style={{ width: "100%", padding: "14px 18px", border: "2px solid #e2e8f0", borderRadius: "10px", fontSize: "15px", color: "#334155", background: "#f8fafc", fontFamily: "inherit", outline: "none", cursor: "pointer" }}>
              ...
              {touched.department && errors.department && <span className="error-text">{errors.department}</span>}
            </div>

            <div className="form-row">
              <div className="form-group">
                <label>Year</label>
                <select name="year" value={student.year} onChange={handleChange} onBlur={handleBlur} required
                  className={touched.year && errors.year ? "error-input" : ""}
                  style={{ width: "100%", padding: "14px 18px", border: "2px solid #e2e8f0", borderRadius: "10px", fontSize: "15px", color: "#334155", background: "#f8fafc", fontFamily: "inherit", outline: "none", cursor: "pointer" }}>
                ...
                {touched.year && errors.year && <span className="error-text">{errors.year}</span>}
              </div>
              <div className="form-group">
                <label>Semester</label>
                <select name="semester" value={student.semester} onChange={handleChange} onBlur={handleBlur} required
                  className={touched.semester && errors.semester ? "error-input" : ""}
                  style={{ width: "100%", padding: "14px 18px", border: "2px solid #e2e8f0", borderRadius: "10px", fontSize: "15px", color: "#334155", background: "#f8fafc", fontFamily: "inherit", outline: "none", cursor: "pointer" }}>
                  <option value="" disabled>Select Semester</option>
                  <option value="1">Semester 1</option>
                  <option value="2">Semester 2</option>
                </select>
                {touched.semester && errors.semester && <span className="error-text">{errors.semester}</span>}
              </div>
            </div>
            ...
            <div className="form-group">
              ...
              <textarea name="address" placeholder="Your full address"
                value={student.address} onChange={handleChange} onBlur={handleBlur} required rows={3}
                className={touched.address && errors.address ? "error-input" : ""}></textarea>
        ...
        {/* Success Popup */}
        {showSuccessPopup && (
          <div className="popup-overlay" onClick={() => { setShowSuccessPopup(false); navigate("/StudentProfile"); }}>
            <div className="popup-modal success-popup" onClick={(e) => e.stopPropagation()}>
              ...
              <button className="popup-close-btn" onClick={() => { setShowSuccessPopup(false); navigate("/StudentProfile"); }}>OK</button>
            </div>
          </div>
        )}
