...
import Gallery from "../RANGA/Gallery";
import About from "../RANGA/AboutR";
...
const StudentDashboard = () => {
  ...
  const handleLeaveSubmit = async () => {
    ...
    // 3. Reason length
    if (leaveForm.reason.length < 5) {
      setLeaveSubmitStatus('error');
      setLeaveSubmitMsg('Reason must be at least 5 characters long.');
      return;
    }
  ...
  const handleChange = (e) => {
    ...
    if (name === "studentId") {
      ...
      if (/^$|^I$|^IT$|^IT\d{0,8}$/.test(formattedValue)) {
  ...
  const handleSave = async () => {
    ...
    if (formData.studentId && !itNumberRegex.test(formData.studentId)) {
      setSaveError("IT Number must start with 'IT' followed by exactly 8 digits.");
      return;
    }

    const nameRegex = /^[a-zA-Z\s]+$/;
    if (formData.fullName && !nameRegex.test(formData.fullName)) {
      setSaveError("Full Name must contain only English letters and spaces.");
      return;
    }
  ...
  const fields = [
    { name: 'itNumber', label: 'IT Number', icon: 'M10 6H5a2 2 0 00-2 2v9a2 2 0 002 2h14a2 2 0 002-2V8a2 2 0 00-2-2h-5m-4 0V5a2 2 0 114 0v1m-4 0a2 2 0 104 0m-5 8a2 2 0 100-4 2 2 0 000 4zm0 0c1.306 0 2.417.835 2.83 2M9 14a3.001 3.001 0 00-2.83 2M15 11h3m-3 4h2', full: false },
  ...
  const currentPhoto = photoPreview || studentData._localPhoto
    || (studentData.profilePhoto ? (studentData.profilePhoto.startsWith('http') ? studentData.profilePhoto : `${API_BASE}/${studentData.profilePhoto}`) : null)
    || `https://ui-avatars.com/api/?name=${encodeURIComponent(displayName)}&size=120&background=2563eb&color=fff`;
  ...
  const navItems = [
    ...
    { section: 'payments', label: 'Payment Details', group: 'STUDENT', d: 'M3 10h18M7 15h1m4 0h1m-7 4h12a3 3 0 003-3V8a3 3 0 00-3-3H6a3 3 0 00-3 3v8a3 3 0 003 3z' },
    { section: 'complaints', label: 'Submit Complaint', group: 'STUDENT', d: 'M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 17c-.77 1.333.192 3 1.732 3z' },
    ...
    { section: 'identity', label: 'Virtual ID', group: 'STUDENT', sublabel: 'Digital Pass', d: 'M15 7a2 2 0 012 2m4 0a6 6 0 01-7.743 5.743L11 17H9v2H7v2H4a1 1 0 01-1-1v-2.586a1 1 0 01.293-.707l5.964-5.964A6 6 0 1121 9z' },
  ...
  const renderContent = () => {
    switch (activeSection) {
      ...
      case 'dashboard':
        return (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
            ...
            <div style={{ ...styles.dashboardPanel, padding: '2rem', display: 'flex', alignItems: 'center', gap: '2.5rem', background: 'linear-gradient(135deg, rgba(255,255,255,0.7), rgba(219,234,254,0.4))', backdropFilter: 'blur(16px)', border: '1px solid rgba(255,255,255,0.5)' }}>
               <div style={{ position: 'relative' }}>
                  <div style={{ width: '120px', height: '120px', borderRadius: '50%', border: '4px solid #fff', boxShadow: '0 10px 25px rgba(30,58,138,0.15)', overflow: 'hidden', background: '#f8fafc' }}>
                    <img src={currentPhoto} alt="Profile" style={{ width: '100%', height: '100%', objectFit: 'cover' }} onError={(e) => { e.target.src = `https://ui-avatars.com/api/?name=${encodeURIComponent(displayName)}&size=120&background=2563eb&color=fff`; }} />
                  </div>
                  <div style={{ position: 'absolute', bottom: '5px', right: '5px', width: '24px', height: '24px', background: '#10b981', border: '3px solid #fff', borderRadius: '50%' }}></div>
               </div>
               <div>
                  <h1 style={{ fontSize: '1.8rem', fontWeight: 800, color: '#1e3a8a', marginBottom: '0.25rem', fontFamily: "'Plus Jakarta Sans', sans-serif" }}>Welcome Back, {displayName.split(' ')[0]}! 👋</h1>
                  <p style={{ color: '#64748b', fontSize: '1rem', fontWeight: 500 }}>Student ID: <span style={{ color: '#3b82f6', fontWeight: 700 }}>{studentData.itNumber || 'N/A'}</span> · {studentData.department || 'SLIIT'}</p>
            ...
            <div style={styles.dashboardPanel} className="dashboard-panel">
              <div style={styles.panelHeader}><div style={styles.panelHeaderAccent}></div><h2 style={styles.panelHeaderH2}>Student Profile Overview</h2></div>
              <div style={styles.detailsSection}>
                <div style={styles.detailsGrid}>
                  {[
                    { label: 'IT NUMBER', value: studentData.itNumber, d: 'M10 6H5a2 2 0 00-2 2v9a2 2 0 002 2h14a2 2 0 002-2V8a2 2 0 00-2-2h-5m-4 0V5a2 2 0 114 0v1m-4 0a2 2 0 104 0m-5 8a2 2 0 100-4 2 2 0 000 4zm0 0c1.306 0 2.417.835 2.83 2M9 14a3.001 3.001 0 00-2.83 2M15 11h3m-3 4h2' },
                    ...
                    { label: 'PHONE NUMBER', value: studentData.phone, d: 'M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z' },
                    ...
                    { label: 'ROOM ASSIGNMENT', value: myBooking ? `Room ${myBooking.roomNumber || myBooking.room}` : 'Not Booked', d: 'M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6' },
                  ].map(({ label, value, d }) => (
                    <div className="detail-card" style={{ ...styles.detailCard, padding: '1.25rem 1.75rem', background: 'rgba(255,255,255,0.4)' }} key={label}>
                      <div style={{ ...styles.detailCardIcon, width: '44px', height: '44px', fontSize: '1.2rem' }} className="detail-card-icon"><svg width="22" height="22" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d={d} /></svg></div>
                      <div style={styles.detailCardContent}><span style={{ ...styles.detailLabel, fontSize: '0.7rem' }}>{label}</span><span style={{ ...styles.detailValue, fontSize: '1rem', color: '#1e293b' }}>{value || '—'}</span></div>
                    </div>
                  ))}
                </div>
              </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.5rem' }}>
              <div style={{ ...styles.dashboardPanel, padding: '1.5rem', background: 'rgba(219, 234, 254, 0.5)', border: '1px solid rgba(59, 130, 246, 0.2)' }}>
                 <div style={{ fontWeight: 800, color: '#1e40af', marginBottom: '0.5rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                    <span style={{ fontSize: '1.2rem' }}>🔔</span> RECENT NOTIFICATIONS
                 </div>
                 <div style={{ fontSize: '0.9rem', color: '#1e3a8a' }}>
                    You have {notifUnreadCount} unread notifications. Visit the Notification tab to stay updated.
                 </div>
              </div>
              <div style={{ ...styles.dashboardPanel, padding: '1.5rem', background: 'rgba(220, 252, 231, 0.5)', border: '1px solid rgba(16, 185, 129, 0.2)' }}>
                 <div style={{ fontWeight: 800, color: '#065f46', marginBottom: '0.5rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                    <span style={{ fontSize: '1.2rem' }}>📋</span> LEAVE STATUS
                 </div>
                 <div style={{ fontSize: '0.9rem', color: '#064e3b' }}>
                    {myLeaves.length > 0 ? `Your last leave request was ${myLeaves[0].status}.` : "You haven't submitted any leave requests yet."}
                 </div>
              </div>
            </div>
          </div>
              ...
              {activeLeaveTab === 'history' && (
                <div style={styles.leaveHistoryBody}>
                  ...
                  {!leavesLoading && myLeaves.length > 0 && myLeaves.map((leave, idx) => {
                    ...
                    const days = calcDays(leave.startDate || leave.fromDate, leave.endDate || leave.toDate);
                    ...
                    return (
                      <div key={leave._id || idx} className="leave-card" style={styles.leaveCard}>
                        <div style={styles.leaveCardLeft}>
                          ...
                          <p style={styles.leaveCardDates}>📅 {fmt(leave.startDate || leave.fromDate)} → {fmt(leave.endDate || leave.toDate)}</p>
      ...
      case 'identity':
        return <ResidentIDCard student={studentData} roomInfo={myBooking ? `Block ${myBooking.block} · Room ${myBooking.roomNumber || myBooking.room}` : 'Not Assigned'} />;
        ...
        <div style={styles.dashboardLayout} className="dashboard-layout">
              ...
              {['MAIN', 'STUDENT'].map(group => (
                <div style={styles.sidebarSection} key={group}>
                  ...
                  {navItems.filter(i => i.group === group).map(({ section, label, sublabel, d }) => (
                    <button key={section}
                      onClick={() => { 
                        if (section === 'complaints') { navigate('/complaint-dashboard'); return; }
                        if (section === 'payments') { navigate('/student-payments'); return; }
          ...
          <main className="dashboard-main" style={styles.dashboardMain}>
            ...
            <button className="logout-btn" style={styles.logoutBtn}
              onClick={() => { localStorage.removeItem('studentToken'); localStorage.removeItem('studentData'); navigate('/SignIn'); }}>
              Logout
            </button>
