...
const registerUsers = async (req, res) => {
    try {
        ...
        const finalItNumber = studentId || itNumber;
        const finalProfilePhoto = req.file ? req.file.path.replace(/\\/g, '/') : '';
        
        console.log("Registration request received:", { studentId, finalItNumber, email });
        
        // Validate required fields
        if (!finalItNumber || !fullName || !email || !phone || !password || !confirmPassword || !department || !address) {
            console.warn("Validation failed: Missing fields", { finalItNumber, fullName, email, phone, password: !!password, confirmPassword: !!confirmPassword, department, address });
        ...
        const existingITNumber = await User.findOne({ itNumber: finalItNumber });
        if (existingITNumber) {
            return res.status(400).json({ 
                success: false,
        ...
        const token = jwt.sign(
            { 
                ...
                yearSemester: savedUser.yearSemester
            },
            JWT_SECRET,
            { expiresIn: "7d" }
        );
